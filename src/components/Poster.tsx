...
export default function Poster({ id, title, poster_path, vote_average }) {
  return (
    <Link href={`/movie-detail/${id}`} key={id} prefetch>
      <div className="group relative overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 cursor-pointer">
        <Image
        ...
        <div className="absolute bottom-2 left-2 right-2 text-white text-center font-medium truncate bg-black/30 backdrop-blur-sm rounded px-2 py-1">
          {title}
        </div>
        <span
          className={`${
